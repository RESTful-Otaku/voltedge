<script>
	...
	onMount(() => {
		if (simulationId) {
			simulation = $simulationStore.simulations.find(s => s.id === simulationId);
			if (simulation) {
		...
		const unsubscribe = simulationStore.subscribe((store) => {
			if (simulationId) {
				const updatedSimulation = store.simulations.find(s => s.id === simulationId);
	...
	function initializeVisualization() {
		...
		linkElements = zoomGroup.append('g')
			.attr('class', 'links')
			.selectAll('line')
			.data(links)
			.join('line')
		...
		nodeElements = zoomGroup.append('g')
			.attr('class', 'nodes')
			.selectAll('circle')
			.data(nodes)
			.join('circle')
		...
		// Create labels
		labelElements = zoomGroup.append('g')
			.attr('class', 'labels')
			.selectAll('text')
			.data(nodes)
			.join('text')
	...
	function updateGridState() {
		...
		gridState.totalGeneration = simulation.config.power_plants
			.reduce((sum, plant) => sum + (plant.current_output_mw || 0), 0);
		
		gridState.totalConsumption = gridState.totalGeneration * (0.8 + Math.random() * 0.4);
		...
		simulation.config.power_plants.forEach(plant => {
			if (plant.is_operational) {
				plant.current_output_mw = Math.max(0, plant.current_output_mw + (Math.random() - 0.5) * 10);
				plant.current_output_mw = Math.min(plant.current_output_mw, plant.max_capacity_mw);
			}
		});
	}
	...
	function handleNodeHover(event, d) {
		...
		d3.select(event.target).transition().duration(200).attr('r', getNodeRadius(d) * 1.2);
	}
	
	function handleNodeLeave(event, d) {
		// Hide tooltip or remove highlight
		d3.select(event.target).transition().duration(200).attr('r', getNodeRadius(d));
	}
...
<div class="grid-visualization">
	...
	<div class="flex items-center justify-between mb-4">
		<div class="flex items-center space-x-4">
			<div class="flex items-center space-x-2">
				<div class="w-3 h-3 rounded-full {connectionStore.connected ? 'bg-green-500' : 'bg-red-500'}"></div>
	...
	<!-- Grid State Info -->
	<div class="grid grid-cols-4 gap-4 mb-4 p-4 bg-white rounded-lg border border-gray-200">
		<div class="text-center">
			<div class="text-2xl font-bold text-green-600">{gridState.totalGeneration.toFixed(1)}</div>
			<div class="text-sm text-gray-600">Generation (MW)</div>
		</div>
		<div class="text-center">
			<div class="text-2xl font-bold text-blue-600">{gridState.totalConsumption.toFixed(1)}</div>
			<div class="text-sm text-gray-600">Consumption (MW)</div>
		</div>
		<div class="text-center">
			<div class="text-2xl font-bold text-purple-600">{gridState.frequency.toFixed(2)}</div>
			<div class="text-sm text-gray-600">Frequency (Hz)</div>
		</div>
		<div class="text-center">
			<div class="text-2xl font-bold text-orange-600">{gridState.activeFailures.length}</div>
	...
	<div class="mt-4 p-4 bg-white rounded-lg border border-gray-200">
		...
		<div class="grid grid-cols-2 gap-4">
			<div>
				<h4 class="text-sm font-medium text-gray-700 mb-2">Power Plant Types</h4>
				<div class="space-y-1">
