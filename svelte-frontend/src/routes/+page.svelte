...
<svelte:head>
  ...
  <meta
    name="description"
    content="Advanced energy grid simulation platform with real-time monitoring, fault injection, and comprehensive analytics."
  />
</svelte:head>
  ...
  <section class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
    <div class="text-center">
      ...
      <p class="text-gray-600 mb-8 max-w-2xl mx-auto">
        Create your first energy grid simulation and explore the power of
        real-time grid modeling, fault injection, and comprehensive analytics.
      </p>
    ...
    <section class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {#each $simulationStore.simulations.slice(0, 6) as simulation}
          <button
            class="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow cursor-pointer w-full text-left"
            on:click={() => goto(`/simulations/${simulation.id}`)}
            on:keydown={(e) =>
              e.key === "Enter" && goto(`/simulations/${simulation.id}`)}
